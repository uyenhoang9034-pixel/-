...
export default {name:'confession_approve',async execute(interaction,client,args){
 ...
 try{const u=await client.users.fetch(d.authorId);await u.send({content:`${CONFESSION.emojiApproved} **Confession của bạn đã được duyệt!**\n\n${CONFESSION.emojiHeart} Confession #${d.number} đã được đăng.\n\n**Chế độ:** ${CONFESSION.emojiMode} ${d.mode==='public'?'Public':'Ẩn danh'}${d.anonymousCode?`\n**Mã ẩn danh:** ${d.anonymousCode}`:''}`,components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(`https://discord.com/channels/${d.guildId}/${thread.id}`).setLabel('Xem confession').setEmoji('1546047657963819048'))]});}catch{}
